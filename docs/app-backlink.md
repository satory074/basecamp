...
satory074.com/apps に掲載する全アプリ（GitHub topic `featured-app` 付きリポジトリ）には、アプリ一覧へ戻る**固定バックリンクバー**を設置する。本ドキュメントが唯一の正準仕様。2026-08 に全 9 アプリ（todayai / wcup-tsuuka / tenji / uranai / odekaketenki / produce101japan-ranking / yaruo-dsp / m1-omoroi / hyakuichi）をこの仕様に統一済み。2026-09 に samesaengil を追加（計 10 アプリ）。同月 simple-block（Chrome 拡張。Web ページがないので GitHub Pages の紹介ページ `docs/index.html` を homepage にしてそこへ設置）を追加（計 11 アプリ）。
...
| simple-block | CSS | `docs/index.html`（紹介ページ、インライン `<style>`） | `var(--surface)` = `#fff` | `var(--border)` = `#eff3f4` | `var(--muted)` = `#536471` | `var(--accent)` = `#1565c0` |
...
- **コントラスト**: 全 9 アプリの MUTED 実トークン値で 4.5:1 (AA) 以上を確認済み（uranai `ink/70` ≈ 5.5:1、wcup は light/dark 両方。m1-omoroi は `--muted` が paper-2 上 3.24:1 と未達だったため `color-mix(in srgb, var(--ink) 70%, var(--paper-2))` = 5.73:1 を採用。hyakuichi は `#a89b8c` on `#16213e` = 5.87:1、samesaengil は `--text-dim` `#6c757d` on `#ffffff` = 4.68:1、simple-block は `#536471` on `#ffffff` = 6.12:1）。トークンを変えるときは再確認すること
- **計測パラメータなし・12px・36px バー高**: 維持（12px はフッタ慣行として許容、コントラストで補償）

## 横断監査

```bash
# 11 アプリ全てで 1 ヒットずつ返ること
...
grep -rl --include="*.astro" --include="*.tsx" --include="*.html" "app-backlink-bar" \
  ...
  m1-omoroi/web/src hyakuichi/index.html samesaengil simple-block/docs/index.html
