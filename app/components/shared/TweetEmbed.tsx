    ...
    return (
        ...
        >
            <div className="flex items-center gap-2 mb-2">
                <span className="text-sm font-medium" style={{ color: "var(--color-text)" }}>@satory074</span>
...
export function CategoryBadge({ post }: { post: XTweet }) {
    ...
    if (isRetweet) {
        return (
            <span className="feed-item-badge-chip-icon" style={{ backgroundColor: "#00ba7c" }} aria-label="リポスト">
    ...
    if (post.category === "like") {
        return (
            <span className="feed-item-badge-chip-icon" style={{ backgroundColor: "#f91880" }} aria-label="いいね">
    ...
    if (post.category === "bookmark") {
        return (
            <span className="feed-item-badge-chip-icon" style={{ backgroundColor: "#1d9bf0" }} aria-label="ブックマーク">
                <svg width="12" height="12" viewBox="0 0 24 24" fill="currentColor" stroke="none">
    ...
    // オリジナルPost — 黒背景 + Xロゴ
    return (
        <span className="feed-item-badge-chip-icon" style={{ backgroundColor: "#000000" }} aria-label="投稿">
