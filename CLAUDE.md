...
`.env.local` (gitignore 済) に必要 secret を入れて `npm run dev`。代表的な値の入手元は **Environment Variables** 節を参照。`GCS_BUCKET=basecamp-feeds` を設定すると本番と同じく GCS から feed JSON を読みに行き、未設定だと `public/data/*.json` への fs フォールバックが効く（ローカル開発用、git tracked からは外している）。
...
どちらのパターンも最終的には `RichFeedCard` → `FeedCard` シェル (`app/components/shared/FeedCard.tsx`) を render する。X だけは `HomeFeed.tsx` の専用分岐で `react-tweet` を直接埋め込む例外。
...
- **GCS-backed JSON** (read at runtime via `readFeedJson`、ISR 5分): x (`x-tweets.json`), duolingo (`duolingo-stats.json`), steam (`steam-achievements.json`), spotify (`spotify-plays.json`), booklog (`booklog-feed.json`), filmarks (`filmarks-feed.json`), ff14 (`ff14-character.json`), ff14-achievements (`ff14-achievements-feed.json`), diary (`diary-feed.json`), swarm (`swarm-checkins.json`), applehealth (`applehealth-feed.json`), summaries (`summaries.json`) — すべて `gs://basecamp-feeds/`
- **iOS push ingest**: `POST /api/applehealth/ingest` (Bearer `HEALTHKIT_INGEST_SECRET`) — iPhone (iOS Shortcuts または Health Auto Export アプリ) から直接 JSON を受け、`writeFeedJson()` で GCS に書き込み。スキーマは Health Auto Export 公式形式 (`{data:{workouts:[...]}}`) で、Shortcut 自作 → アプリへの乗り換えがサーバー無改修で可能
...
| `feed-storage.ts` | `readFeedJson()` — GCS から ISR 5min 窓で feed JSON を取得。`readFeedFresh()` — キャッシュ無視で最新値を取る (ingest 内で使用)。`writeFeedJson()` — `@google-cloud/storage` で GCS に PUT (Cloud Run runtime SA に bucket 書き込み権限が必要)。`GCS_BUCKET` 未設定時は `public/data/` への fs フォールバック (ローカル開発用) |
| `applehealth.ts` | `parseHealthAutoExportPayload()` — iOS Shortcut / Health Auto Export からの POST body を `AppleHealthWorkoutEntry[]` に変換。距離単位 (km/mi/m) 正規化、id 自動生成、タイトル組み立て (`5.20 km ランニング` 等) を担当 |
...
### Cloud Run × ISR Gotcha (Firebase App Hosting 移行時に実機検出した罠)
...
### Apple Health (iPhone push)
- **Trigger**: 他フィードと違い GitHub Actions ではなく、**iPhone から直接 Cloud Run の POST endpoint を叩く push 型**
- **Endpoint**: `POST /api/applehealth/ingest` (Bearer 認証、`HEALTHKIT_INGEST_SECRET`)
- **Read**: `GET /api/applehealth` → `gs://basecamp-feeds/applehealth-feed.json` を `readFeedJson` で読み `Post[]` に変換 (`category="workout"|"daily"|"mood"` で UI 種別判別)
- **対応カテゴリ (3 種)**:
    1. **workouts** — 1 セッション 1 entry (時間/距離/カロリー)
    2. **dailyActivity** — 歩数 / エクササイズ時間 / アクティブカロリーを **JST 日付で集約**、1 日 1 entry。id = `applehealth-daily-YYYY-MM-DD`、timestamp は `<dayKey>T23:50:00+09:00` に pin (フィード上 日記より前にソート)
    3. **stateOfMind** — iOS 17+ HKStateOfMindSample。1 ログ 1 entry。`labels`/`valence` は表示、`associations` (factors) は永続化のみで UI 非露出 (privacy)
- **Schema (受信形式)**: Health Auto Export 公式 JSON 互換 — `{data:{workouts:[...], metrics:[{name:"step_count|apple_exercise_time|active_energy", units, data:[{qty,date}]}], stateOfMind:[{id,start,end,kind,labels,associations,valence,valenceClassification}]}}`。距離 km/mi/m + 時間 sec/min/hr + エネルギー kcal/kJ を自動正規化
- **iPhone 側のベストプラクティス**: **Health Auto Export アプリ** ($5–10/Premium) で REST API automation を 1 つ作成し workouts + metrics + stateOfMind をまとめて 1 payload で送る。`Summarize Data: Day` を ON にすれば歩数/エクササイズが 1 日 1 値に集約される (iOS Shortcut 自作は 3 種対応すると過大なため非推奨)
- **Storage**: 各カテゴリ独立の `MAX_*` で trim (workouts/daily 各 1460 entries ≒ 4年、mood 3650 entries)。dedup は **id ベース Map で `incoming で existing 上書き`** — Health Auto Export が当日分を重複送信しても整合
- **read-after-write consistency**: `readFeedFresh()` は ingest 時に **GCS SDK の `file.download()`** を使う (public URL は `Cache-Control: max-age=300` で edge cache されるので直前の write を読み戻せない race 発生実機判明)
- **IAM**: Cloud Run runtime SA `firebase-app-hosting-compute@basecamp-satory074.iam.gserviceaccount.com` に `gs://basecamp-feeds` への `roles/storage.objectAdmin` 付与が必須 (他フィードは GHA の `gha-feed-writer` SA だけ書き込み可だったが、Apple Health は runtime SA も書き込みが必要)
- **Secret Manager**: `HEALTHKIT_INGEST_SECRET` (`openssl rand -hex 32` で生成、`apphosting.yaml` の RUNTIME availability で expose)。App Hosting backend SA `service-130346180231@gcp-sa-firebaseapphosting.iam.gserviceaccount.com` に `secretAccessor` + `secretVersionManager` の両方が必要 (preparer が version pin label を付ける)
- 失敗時の Discord 通知は v2 でも未実装 (Cloud Run logs で確認)
...
HEALTHKIT_INGEST_SECRET=...    # Auth for /api/applehealth/ingest POST (iPhone Shortcut)
...
`s-maxage=300` の Cloud CDN キャッシュにより、push 後 ~5 min はホームを叩いても旧バンドルの HTML が返る。**新しい変更が反映されたか今すぐ確認したい**ときは URL に unique query を付けて origin を直叩き:

```bash
# 新バンドルの etag に変わっているか / 新しい class が含まれるか
curl -sf "https://basecamp-web--basecamp-satory074.asia-east1.hosted.app/?cb=$RANDOM$RANDOM" \
  | grep -oE 'feed-item-[a-z-]+' | sort -u
```

`Cache-Control: no-cache` ヘッダだけでは `s-maxage` を bypass できない（CDN は request の `no-cache` を honor しない）。query を変えるのが確実。Cloud Build の status は `gcloud builds list --project=basecamp-satory074 --region=asia-east1 --limit=3` で確認。
...
- **Secret Manager**: `NEXT_PUBLIC_SUPABASE_URL`, `NEXT_PUBLIC_SUPABASE_ANON_KEY`, `SUPABASE_SERVICE_ROLE_KEY`, `NAITA_SECRET`, `HEALTHKIT_INGEST_SECRET` — `apphosting.yaml` で `availability: BUILD/RUNTIME` 指定。compute SA `firebase-app-hosting-compute@basecamp-satory074.iam.gserviceaccount.com` に `roles/secretmanager.secretAccessor` 付与
- **Runtime GCS write** (Apple Health 専用): compute SA に `roles/storage.objectAdmin` を `gs://basecamp-feeds` に付与する必要がある (他フィードは GHA だけ書くので runtime SA は読み取りだけだが、Apple Health は Cloud Run runtime が直接書き込むので追加 grant 必須)
