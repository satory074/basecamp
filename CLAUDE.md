...
External APIs/RSS/Scraping → /app/api/[platform]/route.ts → JSON response
                                                              ↓
                              *Client.tsx fetch() → FeedPosts → FeedItemCard
```

Homepage: Server-side fetch of all `/api/*` → aggregate + sort → `HomeFeed` → `FeedItemCard` (non-X) / `TweetWithFallback` (X)

Both `HomeFeed` and `FeedPosts` delegate card rendering to `FeedItemCard`. X posts in HomeFeed are the exception — they use `TweetWithFallback` for tweet embeds instead.
...
Platform keys (used in CSS classes, `platformColors`, `platformInitials`) are lowercase without spaces: `hatenabookmark`, `ff14-achievement`, `github`. But `*Client.tsx` `source` props use display names: `"Hatena Bookmark"`, `"FF14 Achievement"`, `"GitHub"`. Two mappings exist:
- `FeedPosts.tsx` `sourceToKey`: display name → platform key
- `FeedItemCard.tsx` `platformDisplayNames`: platform key → display name

When adding a new platform with a multi-word name, update both mappings.
...
- **`FeedItemCard.tsx`**: Unified card component used by both `HomeFeed` and `FeedPosts`. Composes `PlatformBadge`, `Thumbnail`, and `FeedItemMeta`. Includes `platformDisplayNames` mapping for human-readable platform names.
- **`PlatformBadge.tsx`**: 20×20 colored circle with platform initial letter (replaces the old 8px dot)
- **`FeedItemMeta.tsx`**: Platform-specific metadata pills rendered below description (GitHub: language + stars, Booklog: status, Filmarks: rating, HatenaBookmark: user count, Spotify: artist)
