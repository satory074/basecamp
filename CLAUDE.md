...
> ⚠️ **`README.md` は古い** (Next.js 15 / Vercel と書かれているが実際は Next.js 16 / Amplify、存在しない npm script も記載)。**この CLAUDE.md を正とする**。
...
- **Static JSON**: x (`public/data/x-tweets.json`), duolingo (`public/data/duolingo-stats.json`), steam (`public/data/steam-achievements.json`), spotify (`public/data/spotify-plays.json`), booklog (`public/data/booklog-feed.json`), filmarks (`public/data/filmarks-feed.json`), ff14 (`public/data/ff14-character.json`), ff14-achievements (`public/data/ff14-achievements-feed.json`), diary (`public/data/diary-feed.json`), swarm (`public/data/swarm-checkins.json`), summaries (`public/data/summaries.json`)
...
- **Hosting**: AWS Amplify (auto-deploys on push to main, ~3-4 min)
- **Domain**: satory074.com
- **Always run `npm run build` locally before pushing**

### Amplify ビルドコスト運用 (重要)

- 全 update workflow の commit message には **`[skip-cd]` を付ける** — Amplify はそのコミットの自動ビルドを skip する。これがないと cron commit (日 60+) が全部ビルドを起動して月数千円のビルド分課金になる
- `[skip-cd]` でビルドが走らないと、`fs.readFileSync('public/data/*.json')` で読まれる JSON が SSR Lambda にバンドルされた古い版で凍結する。これを防ぐため **EventBridge Scheduler が JST 00:30 に毎日 1 回 Amplify を rebuild する** (script: `scripts/aws-scheduled-build.sh`)
- 構築/撤去/状態確認: `./scripts/aws-scheduled-build.sh {create|delete|status|test}`
- 関連 AWS リソース:
    - IAM Role: `basecamp-amplify-scheduler-role`
    - EventBridge Schedule: `default/basecamp-daily-amplify-build`
    - Amplify App ID: `d3t5r6cslebe5k` (region: ap-northeast-1)
- 月コスト目安: ビルド 30 回 × $0.033 = **約 $1 / ¥150** (旧 $44.54 から 97.8% 削減)

### Amplify ビルド通知 (Discord)

- Amplify ビルド SUCCEED / FAILED を Discord webhook へ通知。Lambda 不要、EventBridge → API Destination で直接 POST
- 構築/撤去/状態確認: `DISCORD_WEBHOOK_URL='https://discord.com/api/webhooks/...' ./scripts/aws-amplify-discord-notifications.sh {create|delete|status|test}`
- 関連 AWS リソース:
    - IAM Role: `basecamp-amplify-discord-events-role`
    - EventBridge Connection: `basecamp-discord-webhook-conn` (API_KEY auth, 実際には使われないダミーヘッダ)
    - EventBridge API Destination: `basecamp-discord-webhook` (Discord webhook URL を保持)
    - EventBridge Rule: `basecamp-amplify-build-status` (`source: aws.amplify`, `jobStatus: [SUCCEED, FAILED]`)
- Discord webhook URL は `create` 時に env var で渡す。再構築時 (`delete` → `create`) も同じ URL を再投入する必要があるため安全な場所に保管しておくこと
- Input Transformer で Amplify event を Discord embed (`title` / `url` / `fields`) に変換。Amplify コンソールへのディープリンク付き
- 通知が来ない場合は Rule の `Invocations` メトリクス (CloudWatch) と API Destination の `InvocationsFailedToBeSentToDeadLetterQueue` を確認
