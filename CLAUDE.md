...
- **逆リンクバー必須**: featured-app にするアプリには satory074.com/apps へ戻る固定バックリンクバーを設置する。正準スニペット・仕様・チェックリストは `docs/app-backlink.md`（全アプリ `app-backlink-bar` クラス付きで横断 grep 可能。2026-08 に全 9 アプリ統一、以降の新規掲載アプリにも都度設置＝現在 10 アプリ）
