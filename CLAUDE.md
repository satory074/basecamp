...
- **Read**: `GET /api/applehealth` → `gs://basecamp-feeds/applehealth-feed.json` を `readFeedJson` で読み `Post[]` に変換
- **Schema (受信形式)**: Health Auto Export 公式 JSON 互換 — `{data:{workouts:[{id,name,start,end,duration,distance:{qty,units},activeEnergyBurned:{qty,units}}]}}`。距離は km/mi/m を自動正規化、id 未指定時は `sha1(name|start)` で生成
- **iPhone 側の選択肢**:
    1. **iOS ショートカット自作** (無料): `Find Workouts` → dictionary 化 → `Get Contents of URL` で POST。Automation で「ワークアウト終了時」or「毎日 23:00」起動。**iPhone がロック中は HealthKit が読めない iOS 仕様**なので unlock 通知が一手間
    2. **Health Auto Export アプリ** ($5–10): Premium の REST automation で同 endpoint に向ければサーバー無改修で動く (schema 互換)
- **Storage**: `MAX_ENTRIES=1460` (~4年分) で古いものから trim。dedup は workout id ベースの Map で `existing → incoming で上書き`
...
- **Secret Manager**: `HEALTHKIT_INGEST_SECRET` (`openssl rand -hex 32` で生成、`apphosting.yaml` の RUNTIME availability で expose)
- 失敗時の Discord 通知は v1 では未実装 (Cloud Run logs で確認)
