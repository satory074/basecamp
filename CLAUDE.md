...
Apple Health 連携は GitHub Pages 移行 (2026-05-18) で削除した。`/api/applehealth/ingest` POST endpoint と専用ページ・サイドバーエントリ・スタイルすべて廃止。

`gs://basecamp-feeds/applehealth-feed.json` は GCS に残存している (バックアップ目的)。再開するなら別アプローチ (Supabase Edge Function 等) を検討すること。

<details>
<summary>削除前の旧仕様（参考）</summary>
