...
- `app/page.tsx` は `revalidate = 21600`（6 時間） — Cloud Run コスト削減のため緩和
...
- キャッシュ: `gs://basecamp-feeds/booklog-cache.json` (30日TTL、dedup merge)
...
共通: 5 concurrent requests (`BATCH_SIZE = 5`), 15s timeout, リトライ3回 (指数バックオフ + ジッター), `gs://basecamp-feeds/*-cache.json` に増分キャッシュ。ページネーション対応スクリプトは全ページキャッシュ済みで停止するインクリメンタル方式。
